...
# Calculate the minimum required buffer size for the hardware
min_buffer_size = AudioRecord.getMinBufferSize(SAMPLE_RATE, CHANNEL_CONFIG, AUDIO_FORMAT)

# Instantiate the Recorder via Android API
...
# Allocate a Python byte array matching the required frame buffer footprint
# 16-bit encoding means 2 bytes per sample chunk
buffer_elements = min_buffer_size // 2
audio_buffer = [0] * buffer_elements

def get_decibels():
    """Reads raw short buffers and computes relative decibels (dBFS)"""
    # Read the data into our buffer list. Returns number of samples read.
    samples_read = recorder.read(audio_buffer, 0, buffer_elements)
    
    if samples_read <= 0:
        return -100.0  # Safe floor fallback value if microphone fails to report data

    # Calculate Root Mean Square (RMS) amplitude over the block
    sum_squares = 0.0
    ...
    # Floor limit to prevent invalid mathematical domain error with math.log10(0)
    if rms < 1.0:
        return -100.0

    # Convert amplitude to Decibels Relative to Full Scale (dBFS)
    # Max possible value for 16-bit signed integer is 32767
...
def main():
    ...
    while running:
        ...
        n = round(display_vol/14)
        m = round(display_vol/16)
